...
def analyze_csv_file(uploaded_file):
    if uploaded_file is not None:
        ...
        if 'columns_removed' not in st.session_state:
            ...
            selected_columns = st.multiselect("Select columns to be removed", options=df.columns)
        ...
        if has_missing_values or has_duplicates:
            ...
            if has_missing_values:
                ...
                for column in missing_values[missing_values > 0].index:
                    ...
                    # Filling with any other value
                    custom_value = st.text_input(f"Any value to fill missing values in '{column}'")
                    if st.button(f"Fill missing values in '{column}' with any value"):
                        if custom_value:
                            df[column].fillna(custom_value, inplace=True)
                            st.session_state['df'] = df
                            st.success(f"Missing values in '{column}' filled with Input value.")
                        else:
                            st.warning("Please provide an appropriate value.")
